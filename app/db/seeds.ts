...
const seed = async () => {
  ...
  const user = await db.user.create({ data: { email: userEmail, name: userName } })
  const collection = await db.collection.create({
    data: { title: "✨ Featured Images", user: { connect: { id: user.id } } },
  })
  for (const resourceUrl of resourceUrls) {
    const resources = await createResourcesForUrl({ resourceUrl }, {} as Ctx)
    for (const resource of resources) {
      await db.collectionItem.create({
        data: { userId: user.id, resourceId: resource.id, collectionId: collection.id },
      })
    }
  }
}
