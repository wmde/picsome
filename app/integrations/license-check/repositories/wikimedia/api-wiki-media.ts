...
export const getAuthorAndLicenseFromWikiMediaEntityUrl = async (
  entityId: string,
  relativeUrl: string
): Promise<AuthorAndLicense | undefined> => {
  // Fetch entity
  ...
  const authorName =
    authorQualifiers !== undefined
      ? readEntityObjectQualifier(authorQualifiers, "P2093")
      : undefined
  const authorUrl =
    authorQualifiers !== undefined
      ? readEntityObjectQualifier(authorQualifiers, "P2699")
      : undefined

  if (typeof authorName !== "string" || typeof authorUrl !== "string") {
    logger.info(`No MediaWiki author metadata for ${entityId}`)
    return undefined
  }
...
const readEntityObjectQualifier = (obj: any, propId: string): string | undefined =>
  obj?.[propId]?.[0]?.datavalue?.value

const readEntityObjectId = (obj: any, propId: string): string | undefined =>
