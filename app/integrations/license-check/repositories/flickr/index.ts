...
import {
  ResourceCreateWithoutDetailsWithoutLicenseInput,
  ResourceDraft,
} from "app/check/mutations/createResource"
...
export async function getFileInfo(id: string) {
  ...
  logger.info("flickr fetch success ", id, response.body)
...
async function getResourceDraftsForUrl(url: string, limit: number = 20): Promise<ResourceDraft[]> {
  ...
  const { url: licenseUrl } = flickrLicenses[license]
  ...
  const author = {
    name: owner.username,
    url: `https://flickr.com/photos/${owner.path_alias}`,
  }

  const resource: ResourceCreateWithoutDetailsWithoutLicenseInput = {
    ...
    resourceUrl: url,
    imgUrl,
    thumbUrl: sizeRequest.body.sizes.size[9].source,
    resolutionX: originalSize.width,
    resolutionY: originalSize.height,
    sizeBytes: 0,
  }

  return [{ resource, licenseUrl }]
}
